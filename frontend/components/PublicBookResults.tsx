...
export function PublicBookResults({
  ...
  if (!identity.valid) {
    return (
      <div className={theme === "light" ? "mt-8 rounded-3xl border border-ps-border bg-ps-surface p-6 text-ps-text-primary" : "mt-8 rounded-3xl border border-amber-300/20 bg-amber-300/10 p-6 text-amber-50"} role="status">
        <p className={theme === "light" ? "text-sm uppercase tracking-[0.22em] text-ps-warning" : "text-sm uppercase tracking-[0.22em] text-amber-100/70"}>Invalid ISBN</p>
        <h1 className="mt-2 text-2xl font-black">That does not appear to be a valid ISBN-10 or ISBN-13.</h1>
        <p className={theme === "light" ? "mt-3 text-sm leading-6 text-ps-text-secondary" : "mt-3 text-sm leading-6 text-amber-100/90"}>
  ...
  return (
    <>
      <div className="mt-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className={theme === "light" ? "text-sm uppercase tracking-[0.25em] text-ps-neutral" : "text-sm uppercase tracking-[0.25em] text-slate-400"}>Exact used-book edition</p>
          <h1 className="mt-2 text-3xl font-black sm:text-4xl">ISBN {identity.normalized}</h1>
          <p className={theme === "light" ? "mt-3 text-sm leading-6 text-ps-text-secondary" : "mt-3 text-sm leading-6 text-slate-300"}>
            {identity.isbn13 ? `ISBN-13 ${identity.isbn13}` : ""}
            {identity.isbn13 && identity.isbn10 ? " · " : ""}
            {identity.isbn10 ? `ISBN-10 ${identity.isbn10}` : ""}
          </p>
          <p className={theme === "light" ? "mt-1 text-sm text-ps-neutral" : "mt-1 text-sm text-slate-500"}>
            Exact ISBN matching · Used Buy It Now copies · Study guides and companion products removed
          </p>
        </div>
        <div className="flex flex-col items-start gap-3 sm:items-end">
          <p className={theme === "light" ? "text-sm text-ps-text-secondary" : "text-sm text-slate-300"}>Live eBay results · Up to 3 standard used copies</p>
          <ShareSearchButton label={`Book ISBN ${identity.normalized}`} bestPrice={bestPrice} theme={theme} />
        </div>
      </div>

      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className={theme === "light" ? "rounded-3xl border border-ps-border bg-ps-surface p-5" : "rounded-3xl border border-white/10 bg-white/[0.05] p-5"}>
          <p className={theme === "light" ? "text-sm text-ps-neutral" : "text-sm text-slate-400"}>eBay candidates</p>
          <p className="mt-2 text-3xl font-black">{data.candidate_count}</p>
        </div>
        <div className={theme === "light" ? "rounded-3xl border border-ps-border bg-ps-surface p-5" : "rounded-3xl border border-white/10 bg-white/[0.05] p-5"}>
          <p className={theme === "light" ? "text-sm text-ps-neutral" : "text-sm text-slate-400"}>Standard used copies</p>
          <p className="mt-2 text-3xl font-black">{data.standard_count}</p>
        </div>
        <div className={theme === "light" ? "rounded-3xl border border-ps-border bg-ps-surface p-5" : "rounded-3xl border border-white/10 bg-white/[0.05] p-5"}>
          <p className={theme === "light" ? "text-sm text-ps-neutral" : "text-sm text-slate-400"}>Collectible copies separated</p>
          <p className="mt-2 text-3xl font-black">{data.collectible_count}</p>
        </div>
        <div className={theme === "light" ? "rounded-3xl border border-ps-border bg-ps-surface p-5" : "rounded-3xl border border-white/10 bg-white/[0.05] p-5"}>
          <p className={theme === "light" ? "text-sm text-ps-neutral" : "text-sm text-slate-400"}>Book bundles separated</p>
      ...
      <OpenLibraryAvailabilityCard availability={openLibrary} theme={theme} />

      {data.collectible_results.length ? (
        <section className={theme === "light" ? "mt-10 rounded-3xl border border-ps-border bg-ps-surface p-5 sm:p-6" : "mt-10 rounded-3xl border border-purple-300/20 bg-purple-300/[0.06] p-5 sm:p-6"}>
          <p className={theme === "light" ? "text-sm uppercase tracking-[0.2em] text-ps-info" : "text-sm uppercase tracking-[0.2em] text-purple-200"}>Collectible alternatives</p>
      ...
      {data.bundle_results.length ? (
        <section className={theme === "light" ? "mt-10 rounded-3xl border border-ps-border bg-ps-surface p-5 sm:p-6" : "mt-10 rounded-3xl border border-amber-300/20 bg-amber-300/[0.06] p-5 sm:p-6"}>
          <p className={theme === "light" ? "text-sm uppercase tracking-[0.2em] text-ps-warning" : "text-sm uppercase tracking-[0.2em] text-amber-200"}>Multi-book alternatives</p>
      ...
      {Object.keys(data.rejection_reasons).length ? (
        <details className={theme === "light" ? "mt-8 rounded-3xl border border-ps-border bg-ps-surface" : "mt-8 rounded-3xl border border-white/10 bg-white/[0.04]"}>
          <summary className={theme === "light" ? "cursor-pointer list-none p-5 text-sm font-bold text-ps-accent-hover" : "cursor-pointer list-none p-5 text-sm font-bold text-cyan-100"}>Why listings were removed ↓</summary>
          <div className={theme === "light" ? "border-t border-ps-border p-5 text-sm text-ps-text-secondary" : "border-t border-white/10 p-5 text-sm text-slate-300"}>
