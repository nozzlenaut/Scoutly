...
export default async function KehAdminPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-950 px-6 py-10 text-white"><div className="mx-auto max-w-[1500px]"><div className="flex flex-wrap items-center justify-between gap-4"><Link href={`/admin?token=${encodeURIComponent(token)}`} className="text-sm text-cyan-200">← Testing dashboard</Link><Link href={`/admin/qa?token=${encodeURIComponent(token)}`} className="text-sm text-slate-400">Search QA</Link></div><div className="mt-8"><p className="text-sm uppercase tracking-[0.25em] text-slate-500">PriceSift admin</p><h1 className="mt-2 text-4xl font-black">KEH shadow feed</h1><p className="mt-3 max-w-4xl text-slate-400">Imports KEH camera bodies and interchangeable lenses, matches only the camera QA pilot products, and keeps every result private until the feed earns its way onto the public stage.</p></div><AdminKehDashboard initialOverview={overview} token={token}/><SiteFooter /></div></main>;
}
