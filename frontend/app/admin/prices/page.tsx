    ...
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-xl">
        ...
        <section className="mt-10 rounded-3xl border border-white/10 bg-white/[0.05] p-6">
          ...
          <AdminLoginForm next="/admin/prices" />
        </section>
      </div>
    </main>
  );
...
export default async function PriceHistoryPage({ searchParams }: { searchParams: Promise<{ invalid?: string }> }) {
  const params = await searchParams;
  const token = await getAdminToken();
  if (!token) return <PriceGate invalid={params.invalid === "1"} />;
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-[1500px]">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <Link href={`/admin`} className="text-sm text-cyan-200 hover:text-cyan-100">← Testing dashboard</Link>
          <Link href={`/admin/qa`} className="text-sm text-slate-400 hover:text-slate-200">Search QA</Link>
        </div>
        <div className="mt-8">
